...
const ToastComponent: React.FC<{ text: string; duration: number }> = memo(({ text, duration }) => {
    ...
    return (
        <div className="relative flex justify-center items-center min-w-[240px] h-[40px] px-[32px] bg-[#445159] text-white rounded overflow-hidden">
            {text}
            {duration ? (
                <a.div className="absolute bottom-0 w-full h-[4px] bg-gradient-to-l from-[#15C184] to-[#2959B4]" style={props} />
            ) : null}
        </div>
    );
});
