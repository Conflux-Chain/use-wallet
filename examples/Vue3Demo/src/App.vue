...
import { useStatus, useAccount, useChainId, useBalance, connect, Unit, sendTransaction, typedSign, switchChain } from '@cfxjs/use-wallet-vue3/ethereum/Fluent';
...
const isChainMatch = computed(() => chainId.value === TargetChain);
    ...
    <div>
        ...
        <div v-else-if="status === 'active'">
            <template v-if="isChainMatch">
                ...
                <button @click="handleTypedSign">typed sign</button>
            </template>
            <template v-else>
                <p>Current Chain is not Target Chain -- {{ TargetChain === TargetChains.eSpaceMainnet ? 'eSpaceMainnet' : 'eSpaceTestnet' }}</p>
                <button @click="switchChain(`0x${(+TargetChain).toString(16)}`)">Switch to Target Chain</button>
            </template>
        </div>
    </div>
</template>
