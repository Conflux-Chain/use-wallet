...
const transitions = {
    en: {
        ...
        step3: `As you can see in the example in Basic Usage, when the status is 'in-detecting', no content should be rendered, and it is a bad user experience to render transition content for a very short time. However, this may cause the content block to jitter when refreshing the page, so a completeDetect method is also provided to call ReactDOM.render after the detection is complete, see Others for details.`,
    ...
    zh: {
        ...
        step3: `可以看到 Basic Usage 中的例子， status 为 'in-detecting' 的时候，理应不渲染任何内容，为极短的时间渲染过渡内容反而是一种不好的用户体验。但是这可能会导致刷新页面时内容块的抖动，所以也提供了一个 completeDetect 方法用以在 detect 完成后再去调用 ReactDOM.render，具体见 Others。`,
