...
const transitions = {
    en: {
        ...
        step2: `The return of useBalance is an instance object of Unit, whose value unit is the smallest unit of the currency ('drip' in conflux | 'wei' in ethereum). The details of how to operate this can be found in the Unit description in Utils.`
    },
    zh: {
        step1: `useBalance 返回当前钱包中账户的余额。如果是 status 为 'not-active' 的未授权连接状态，返回值为 undefined。`,
        step2: `useBalance 的返回单位为 Unit 的实例对象，其内的 value 单位为币种的最小单位（'drip' in conflux | 'wei' in ethereum）。具体的操作方法可以见下 Utils 中 Unit 具体介绍。`
    },
} as const;
